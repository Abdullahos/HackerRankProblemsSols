    ...
    static int queensAttack(int n, int k, int r_q, int c_q, int[][] obstacles) {
        ...
        for(int r = 0 ; r <k ; r++){
                ...
                //if obs on diagonals:
                    ...
                    if(obstacles[r][0]>r_q){
                        //dia up R
                        if(obstacles[r][1]>c_q && obstacles[r][1]-c_q-1<nearestDiaUpR)         {
                            nearestDiaUpR  = obstacles[r][1]-c_q-1;
                        }
                        //dia up l
                        else if(obstacles[r][1]<c_q && c_q-obstacles[r][1]-1<nearestDiaUpL)         {
                        nearestDiaUpL  = c_q-obstacles[r][1]-1;
                        }
                    }
                       //dia down
                        else{
                            //down l
                            if(obstacles[r][1]<c_q && c_q-obstacles[r][1]-1<nearestDiaDownL)           {
                                nearestDiaDownL = c_q-obstacles[r][1]-1;
                            }
                             //down r
                             else if(obstacles[r][1]>c_q && obstacles[r][1]-c_q-1<nearestDiaDownR)    {
                                nearestDiaDownR = obstacles[r][1]-c_q-1;
                             }
                        }
                }
        }//for
